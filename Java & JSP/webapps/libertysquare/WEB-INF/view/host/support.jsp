...
<% List<Support> supportList = dao.getHostedSupportList(hostID); %>
<% if(supportList == null || supportList.isEmpty()) { %>
	<div class="host-no-event-container">아직 후원행사가 없습니다. 기대해주세요 👀</div>
<% } else { %>
	<div class="host-event-container">
		<% for(Support support : supportList) { %>
			<a class="host-event-item" href="<%= "/event/" + support.getID() %>">
				<div class="host-event-item-head">
					<div class="host-event-item-img" style="<%="background: url(" + support.getCoverPath() + ") center center / cover no-repeat"%>"></div>
					...
					<div class="host-event-item-cover">
						<div class="host-event-item-cover-inner">
							...
							<span class="host-event-item-cover-text price"><%= 123 %></span><!-- 가격 -->
						</div>
					</div>
				</div>
				<div class="host-event-item-body">
					<div class="host-event-item-title"><%= support.getTitle() %></div>
