...
/*
	public static void setRSAKeyAttributes(KeyPair keyPair, HttpSession session) throws Exception {
		removeRSAKeyAttributes(session);
		...
		session.setAttribute(PUBLIC_KEY_MODULUS, publicSpec.getModulus().toString(16));
		session.setAttribute(PUBLIC_KEY_EXPONENT, publicSpec.getPublicExponent().toString(16));
	}
*/
	public static void removeRSAKeyAttributes(HttpSession session) {
		session.removeAttribute(PRIVATE_KEY);
		session.removeAttribute(PUBLIC_KEY_MODULUS);
		session.removeAttribute(PUBLIC_KEY_EXPONENT);
	}
}
